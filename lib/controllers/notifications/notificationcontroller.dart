...
class NotificationController extends GetxController{
  ...
  Future<void> getAllNotifications(String token) async {
    try{
      isLoading = true;
      const url = "http://127.0.0.1:8000/get_my_notifications/";
  ...
  Future<void> getAllUnReadNotifications(String token) async {
    try{
      isLoading = true;
      const url = "http://127.0.0.1:8000/get_my_unread_notifications/";
      var myLink = Uri.parse(url);
      final response =
      await http.get(myLink, headers: {"Authorization": "Token $token"});
      if (response.statusCode == 200) {
  ...
  Future<void> readNotifications(String token) async {
    try{
      isLoading = true;
      const url = "http://127.0.0.1:8000/read_notification/";
      var myLink = Uri.parse(url);
      final response =
      await http.get(myLink, headers: {"Authorization": "Token $token"});
      if (response.statusCode == 200) {
